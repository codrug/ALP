  ...
  <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-black/80 backdrop-blur-md">
    <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
      <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.location.reload()}>
        ...
        <span className="font-extrabold tracking-tighter text-xl uppercase">Master</span>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-6 text-center">
      ...
      <div className="flex flex-wrap justify-center items-center gap-12 md:gap-24 opacity-40 grayscale transition-all hover:grayscale-0">
        {['USMLE', 'BAR EXAM', 'MCAT', 'NCLEX', 'CPA'].map((exam) => (
          <span key={exam} className="text-2xl font-black tracking-tighter text-gray-200">{exam}</span>
        ))}
      </div>
    </div>
  </section>
    ...
    <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20">
      ...
      <div className="space-y-6">
        {[
          ...
          <div key={idx} className="bg-white/[0.03] border border-white/10 p-6 rounded-xl flex gap-6 group hover:border-amber-500/50 transition-all">
            <div className="text-amber-500 pt-1 group-hover:scale-110 transition-transform">
              {item.icon}
            </div>
            <div>
...
const MasteryLoop: React.FC = () => (
  <section className="py-32 px-6 bg-black relative">
    <div className="max-w-7xl mx-auto text-center mb-20">
      <h2 className="text-5xl font-extrabold tracking-tighter mb-6">The 5-Step Mastery Loop</h2>
      <p className="text-gray-400 max-w-2xl mx-auto font-light leading-relaxed">
        The rigorous process that transforms candidates into experts.
      </p>
    </div>

    <div className="max-w-4xl mx-auto relative pt-10 pb-10">
      {/* Central Line */}
      ...
      {[
        ...
        <div key={idx} className={`flex items-center gap-12 md:gap-0 mb-16 relative ${idx % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
          ...
          <div className="hidden md:flex flex-1 justify-center">
            <span className="text-[10px] font-bold tracking-[0.3em] text-gray-700">{item.label}</span>
          </div>

          {/* Node */}
          <div className="relative z-10 w-12 h-12 bg-[#1a1a1a] border border-white/10 rounded-lg flex items-center justify-center text-amber-500 shadow-xl">
             {item.icon}
          </div>
  ...
  <section className="py-32 px-6">
    <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-8">
      {[
        ...
        <div key={idx} className="bg-[#111] border border-white/5 p-10 rounded-xl hover:border-amber-500/30 transition-all flex flex-col h-full">
          ...
          <div className="text-[10px] font-bold tracking-widest text-gray-700 uppercase">{item.tag}</div>
        </div>
      ))}
    </div>
  </section>
...
// --- The 80% Rule ---
const The80PercentRule: React.FC = () => (
  <section className="py-32 px-6">
    <div className="max-w-7xl mx-auto bg-white/[0.02] border border-white/5 rounded-3xl p-12 md:p-24 flex flex-col md:flex-row items-center gap-20 overflow-hidden relative">
      ...
      <div className="flex-1 relative z-10">
        <h2 className="text-5xl font-extrabold tracking-tighter mb-8 leading-tight">The 80% Rule</h2>
      ...
      <div className="flex-1 flex justify-center items-center relative z-10">
        <div className="relative w-80 h-80 flex items-center justify-center">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center text-center -translate-y-4">
            <span className="text-7xl font-black tracking-tighter mb-1 text-white">80%</span>
            <span className="text-[10px] font-bold tracking-[0.3em] text-gray-500 uppercase">Proficiency</span>
          </div>
          
          {/* Threshold Badge: Adjusted position to clear the text area completely */}
          <div className="absolute bottom-[80px] left-1/2 -translate-x-1/2 bg-amber-500 text-black px-4 py-1.5 rounded-full text-[10px] font-bold tracking-widest uppercase shadow-lg shadow-amber-500/20 whitespace-nowrap">
            THRESHOLD MET
          </div>
        </div>
      </div>
  ...
  <footer className="py-20 px-6 border-t border-white/5">
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12">
      <div className="flex items-center gap-2">
        ...
        <span className="font-extrabold tracking-tighter text-lg uppercase">Master</span>
      </div>
